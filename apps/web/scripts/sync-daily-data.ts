...
import { ensureDailyDataFresh, runDailyDataRefresh } from "../lib/server/daily-data-refresh";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const result = process.argv.includes("--if-stale")
      ? await ensureDailyDataFresh(sql)
      : await runDailyDataRefresh(sql, { trigger: "manual" });
